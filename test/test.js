...
describe("USBInfo", function() {
    ...
    it("Async function will throw an error if something wrong with the exec command", function(done) {
        var handler = this.exec.callsArgWith(1, new Error("Something went wrong"), null, null);
        asustor.usbInfo(function(error, data) {
            expect(error.message).to.be.equal("Something went wrong");
            expect(data).to.be.deep.equal({ drives: [], totalAvailableSpace: 0 });
            done();
        });
    });
